...
const SearchBlood = () => {
  ...
  const handleData = async (uId) => {
    localStorage.setItem("bankId", uId);
  ...
  const HandleSearch = async () => {
    ...
    try {
      ...
      data.forEach((item) => {
          localStorage.setItem(item.data().uId);
        });
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <>
      <div style={{marginTop: '20px'}} className="mainSearch">
        <div>
      ...
      {bbData.length > 0 ? (
        <table>
          ...
          {bbData.map((item) => (
            <tr key={item.data().uId}>
              ...
              <td>
                <button className="button" onClick={() => handleData(item.data().uId)}>
                  Req Blood
                </button>
              </td>
            </tr>
        ...
        <h3 style={{ textAlign: "center" }}>Search With Correct Detail Or </h3>
      )}
    </>
  );
};
